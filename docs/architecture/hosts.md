...
Raycast `storage.ts` serializes mutations with a bounded write lock. Its reset-all flow owns a durable backup key, allowing restore after restart (with Undo available in-session).
