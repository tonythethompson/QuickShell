...
public class Main : IPlugin, IPluginI18n, IContextMenu, ISettingProvider, IReloadable, IDisposable
{
    ...
    private List<ContextMenuResult> BuildShortcutContextMenus(string? shortcutId)
    {
        ...
        [
            ...
            CreateContextMenu("Duplicate shortcut", ShortcutGlyphs.Duplicate, _ =>
            {
                ...
                if (ShortcutEditor.TryShowDialog(duplicate, Shortcuts, _editorFactory ?? throw new InvalidOperationException("Quick Shell plugin is not initialized."), _projectAnalysis ?? throw new InvalidOperationException("Quick Shell plugin is not initialized."), _commandSuggestions ?? throw new InvalidOperationException("Quick Shell plugin is not initialized."), GitOperations, TargetStore, TerminalCatalog, out var message))
                {
    ...
    {
        if (ShortcutEditor.TryShowDialog(shortcut, Shortcuts, _editorFactory ?? throw new InvalidOperationException("Quick Shell plugin is not initialized."), _projectAnalysis ?? throw new InvalidOperationException("Quick Shell plugin is not initialized."), _commandSuggestions ?? throw new InvalidOperationException("Quick Shell plugin is not initialized."), GitOperations, TargetStore, TerminalCatalog, out var message))
        {
            NotifyStatus(message);
            RefreshResults();
        }
