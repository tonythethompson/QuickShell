...
public sealed class CommandSuggestionServiceTests : IDisposable
{
    ...
    [Fact] public void GetPills_DefaultMaxCount_MatchesExplicitMaxSlots() { var scripts = new Dictionary<string, string>(); for (var i = 0; i < 12; i++) { scripts[$"test{i}"] = "vitest run"; scripts[$"build{i}"] = "tsc -b"; scripts[$"dev{i}"] = "vite"; } File.WriteAllText(Path.Join(_root, "package.json"), System.Text.Json.JsonSerializer.Serialize(new { scripts })); _suggestions.ResetForTests(); var omitted = _suggestions.GetPills(_root, [], _projectAnalysis); _suggestions.ResetForTests(); var capped8 = _suggestions.GetPills(_root, [], _projectAnalysis, maxCount: SuggestionPillPresentation.DefaultVisibleSlots); _suggestions.ResetForTests(); var capped16 = _suggestions.GetPills(_root, [], _projectAnalysis, maxCount: SuggestionPillPresentation.MaxSlots); Assert.True(omitted.Count > SuggestionPillPresentation.DefaultVisibleSlots); Assert.Equal(capped16.Count, omitted.Count); Assert.Equal(SuggestionPillPresentation.DefaultVisibleSlots, capped8.Count); Assert.True(omitted.Count > capped8.Count); }
    [Fact] public void BuildDataFields_NeverIncludesOpenDirectoryOnly() { File.WriteAllText(Path.Join(_root, "docker-compose.yml"), "services: {}"); _suggestions.ResetForTests(); var fields = SuggestionPillPresentation.BuildDataFields(_root, [], _projectAnalysis, _suggestions, expandSuggestionPills: false); Assert.Equal("true", fields["ShowSuggestionPills"]); Assert.DoesNotContain(fields, kv => kv.Value == "Open directory only"); }
    ...
    [Fact] public void ApplyPill_FillsFirstEmptyCommandDraft() { File.WriteAllText(Path.Join(_root, "docker-compose.yml"), "services: {}"); var rows = new List<LaunchRowDraft> { new() { LaunchTarget = "default" } }; var pills = _suggestions.GetPills(_root, [], _projectAnalysis); Assert.NotEmpty(pills); Assert.True(_suggestions.ApplyPill(rows, pills[0], "default")); Assert.Single(rows); Assert.False(string.IsNullOrWhiteSpace(rows[0].Command)); }
    public void Dispose()
    {
        _provider.Dispose();
