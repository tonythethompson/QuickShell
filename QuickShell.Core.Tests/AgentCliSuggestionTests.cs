...
public sealed class AgentCliSuggestionTests : IDisposable
{
    ...
    [Fact] public void GetPills_MarkerFallback_Suggests() { File.WriteAllText(Path.Combine(_root, "CLAUDE.md"), "# Claude"); File.WriteAllText(Path.Combine(_root, "GEMINI.md"), "# Gemini"); var p = _suggestions.GetPills(_root, [], _projectAnalysis); Assert.Contains(p, x => x.Command == "claude" && x.Source == "agent-marker"); Assert.Contains(p, x => x.Command == "gemini" && x.Source == "agent-marker"); }
    [Fact] public void GetPills_AgentsMd_MapsToCodex() { File.WriteAllText(Path.Combine(_root, "AGENTS.md"), "# Agents"); var p = _suggestions.GetPills(_root, [], _projectAnalysis); Assert.Contains(p, x => x.Command == "codex"); }
    ...
    [Fact] public void GetPills_MarkerOnly_ReturnsAgent() { File.WriteAllText(Path.Combine(_root, "CLAUDE.md"), "# Claude"); var p = _suggestions.GetPills(_root, [], _projectAnalysis); Assert.Contains(p, x => x.Command == "claude" && x.TaskType == TaskTypeCatalog.Agent); }
    public void Dispose() { _provider.Dispose(); AgentCliCatalog.IsCommandOnPathOverride = _prev; try { Directory.Delete(_root, true); } catch { } }
}
