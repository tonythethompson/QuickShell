...
internal sealed class BenchmarkReport
{
    ...
    public string WriteArtifacts(string? outputDirectory = null)
    {
        ...
        var jsonPath = Path.Join(directory, "quickshell-perf-results.json");
        File.WriteAllText(
            jsonPath,
            JsonSerializer.Serialize(artifact, BenchmarkJsonContext.Default.BenchmarkArtifact));

        var markdownPath = Path.Join(directory, "quickshell-perf-results.md");
    ...
    private static string ResolveDefaultOutputDirectory()
    {
        ...
        while (directory is not null && !File.Exists(Path.Join(directory.FullName, "QuickShell.sln")))
        {
            directory = directory.Parent;
        }

        var root = directory?.FullName ?? AppContext.BaseDirectory;
        return Path.Join(root, "artifacts", "perf");
    }
    ...
    private static string BuildMarkdown(BenchmarkArtifact artifact)
    {
        ...
        sb.AppendLine();
        sb.AppendLine(CultureInfo.InvariantCulture, $"Captured: {artifact.Environment.CapturedAtUtc:yyyy-MM-dd HH:mm:ss} UTC");
        sb.AppendLine(CultureInfo.InvariantCulture, $"OS: {artifact.Environment.OperatingSystem}");
        sb.AppendLine(CultureInfo.InvariantCulture, $"Architecture: {artifact.Environment.ProcessArchitecture}, {artifact.Environment.ProcessorCount} logical processors");
        sb.AppendLine(CultureInfo.InvariantCulture, $"Configuration: {artifact.Environment.Configuration}");
        sb.AppendLine();
        ...
        foreach (var group in artifact.Results.GroupBy(r => r.Category))
        {
            ...
            foreach (var stats in group)
            {
                sb.AppendLine(
                    ...
                    $"| {stats.Name} | {stats.Iterations} | {Format(stats.MedianMs)} | {Format(stats.P95Ms)} | {Format(stats.MinMs)} | {Format(stats.MaxMs)} | {FormatBytes(stats.MeanAllocatedBytes)} |");
            }
                ...
                foreach (var stats in withCounts)
                {
                    var counts = string.Join(
                        ...
                        stats.OperationCounts!.Select(
                            kv => string.Create(CultureInfo.InvariantCulture, $"{kv.Key}={kv.Value}")));
                    sb.AppendLine(CultureInfo.InvariantCulture, $"- **{stats.Name}**: {counts}");
                }
            }
    ...
    private static string FormatBytes(double bytes) =>
        bytes >= 1024 * 1024
            ...
            : bytes >= 1024
                ? (bytes / 1024).ToString("0.##", CultureInfo.InvariantCulture) + " KiB"
