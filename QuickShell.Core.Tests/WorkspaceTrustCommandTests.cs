...
public sealed class WorkspaceTrustCommandTests
{
    ...
    public void Grant_then_confirm_with_token_trusts_workspace()
    {
        ...
        var (services, lifetime, drafts) = CreateServices(repository);
        using (lifetime)
        using (drafts)
        {
            var changed = 0;
    ...
    private static (QuickShellServices Services, QuickShellLifetime Lifetime, ShortcutDraftStore Drafts) CreateServices(IShortcutRepository repository)
    {
...
/// <summary>Serializes tests that use ShortcutRepository (Global\QuickShell_shortcuts_json mutex).</summary>
[CollectionDefinition("ShortcutRepositoryMutex", DisableParallelization = true)]
#pragma warning disable CA1711 // Collection fixture name ends with Collection by xUnit convention.
public sealed class ShortcutRepositoryMutexCollection : ICollectionFixture<object>
