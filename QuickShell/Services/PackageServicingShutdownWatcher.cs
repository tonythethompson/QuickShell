...
internal sealed partial class PackageServicingShutdownWatcher : IDisposable
{
    ...
    private PackageServicingShutdownWatcher(ManualResetEvent exitSignal)
    {
        ...
        if (!_started.Wait(TimeSpan.FromSeconds(5)))
        {
            ...
            SupportDiagnostics.Default.Write("PackageServicingShutdownWatcher", "start-timeout");
        }
    }
    ...
    public void Dispose()
    {
        ...
        if (!_thread.Join(TimeSpan.FromSeconds(2)))
        {
            ...
            SupportDiagnostics.Default.Write("PackageServicingShutdownWatcher", "join-timeout");
            ...
            if (currentHwnd != IntPtr.Zero)
            {
                ...
                if (threadId != 0 && PostThreadMessage(threadId, (uint)WmQuit, IntPtr.Zero, IntPtr.Zero))
                {
                    ...
                    {
                        SupportDiagnostics.Default.Write("PackageServicingShutdownWatcher", "join-timeout-after-quit");
                    }
                }
            }
        }
    ...
    private void RunMessagePump()
    {
        ...
        {
            SupportDiagnostics.Default.WriteException("PackageServicingShutdownWatcher", ex);
            _started.Set();
        }
        finally
        {
    ...
    {
        SupportDiagnostics.Default.Write("PackageServicingShutdownWatcher", "signal-exit", new { reason });
